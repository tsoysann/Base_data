...
class Delivery(db.Model):
    ...
    supplier_id = db.Column(db.BigInteger, ForeignKey('suppliers.supplier_id', ondelete='SET NULL'), nullable=True)
    employee_id = db.Column(db.BigInteger, ForeignKey('employees.employee_id', ondelete='SET NULL'), nullable=True)

    delivery_details = relationship('DeliveryDetail', back_populates='delivery', lazy='dynamic', cascade="all, delete-orphan")
...
class Product(db.Model):
    ...
    order_details = relationship('OrderDetail', back_populates='product', lazy='dynamic',  cascade="all, delete-orphan")
    delivery_details = relationship('DeliveryDetail', back_populates='product', lazy='dynamic', cascade="all, delete-orphan")
...
class Client(db.Model):
    ...
    orders = relationship('Order', back_populates='client', lazy='dynamic', cascade="all, delete-orphan")
...
class Order(db.Model):
    ...
    employee_id = db.Column(db.BigInteger, ForeignKey('employees.employee_id'), nullable=True)
    client_id = db.Column(db.BigInteger, ForeignKey('clients.client_id', ondelete='CASCADE'), nullable=True)
    order_status = db.Column('order_status', db.String(50))

    order_details = relationship('OrderDetail', back_populates='order', lazy='dynamic', cascade="all, delete-orphan")
    payments = relationship('Payment', back_populates='order', lazy='dynamic', cascade="all, delete-orphan")
