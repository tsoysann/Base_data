    ...
    if current_user.role != 'admin':
        ...
        with db.engine.connect() as connection:
            results = connection.execute(
                db.text("SELECT * FROM get_user_orders(:client_id, :start_date, :end_date);"),
                {
        ...
        # Добавляем детали заказов и оплаты
        orders_with_details = []
        for order in orders:
            order_id = order.order_id_res
        ...
        return "Вы не имеете прав доступа к этой странице.", 403
...
def update_profile():
    if current_user.role != 'admin':
        with db.engine.connect() as connection:
            results = connection.execute(db.text("SELECT * FROM user_profile_view;")).first()
            if request.method == 'POST':
            ...
            return render_template('update_profile.html', user=results)
    else:
    ...
    if current_user.role != 'admin':
        ...
        if request.method == 'POST':
            ...
            if user:
                ...
                for product in products:
                    ...
                    if quantity > 0:
                      ...
                      if flower is None and bouquet is None and material is None:
                        db.session.rollback()
                        return render_template('create_order.html', employees=employees, products=products, error=f"ID цветка, букета или материала: {product.product_id} не существует.", search_product = request.form.get('search_product', ''))
                      if flower is None and product.flower_id is not None:
                         db.session.rollback()
                         return render_template('create_order.html', employees=employees, products=products, error=f"ID цветка: {product.flower_id} не существует.", search_product = request.form.get('search_product', ''))
                      if bouquet is None and product.bouquet_id is not None:
                         db.session.rollback()
                         return render_template('create_order.html', employees=employees, products=products, error=f"ID букета: {product.bouquet_id} не существует.", search_product = request.form.get('search_product', ''))
                      if material is None and product.material_id is not None:
                        db.session.rollback()
                        return render_template('create_order.html', employees=employees, products=products, error=f"ID материала: {product.material_id} не существует.", search_product = request.form.get('search_product', ''))
                ...
                return redirect(url_for('user_dashboard'))
        total_price = 0 #сумма заказа
        return render_template('create_order.html', employees=employees, products=products, total_price = total_price, search_product = request.form.get('search_product', ''))
    else:
        return "Вы не имеете прав доступа к этой странице."

@app.route('/user_products_in_orders', methods=['GET', 'POST'])
@login_required
def user_products_in_orders():
    if current_user.role != 'admin':
        user = Client.query.filter_by(client_id=1).first()
        if user:
            # Проверяем, был ли запрос на сброс фильтров
            if 'reset_filter' in request.args:
                min_price = None
                max_price = None
            else:
            ...
            with db.engine.connect() as connection:
                results = connection.execute(
                    db.text("SELECT * FROM get_user_products_in_orders(:client_id, :min_price, :max_price);"),
                    {"client_id": user.client_id, "min_price": min_price, "max_price": max_price},
                )
                order_details = []
                total_spent = 0
                for row in results:
                ...
                return render_template('user_products_in_orders.html', order_details=order_details,
                                       total_spent=total_spent, min_price=min_price, max_price=max_price)
        return "Пользователь не найден"
    else:
        return "Вы не имеете прав доступа к этой странице."

@app.route('/user_deliveries_with_suppliers')
@login_required
def user_deliveries_with_suppliers():
    if current_user.role != 'admin':
         with db.engine.connect() as connection:
              results = connection.execute(
                   db.text("SELECT * FROM get_user_deliveries_with_suppliers();")
              )
              deliveries = []
              last_delivery_id = None
              if results:
                deliveries = list(results)
                if len(deliveries) > 1:
                  last_delivery = sorted(deliveries, key=lambda x: x.delivery_date)[1]
                  last_delivery_id = last_delivery.delivery_id
              return render_template('user_deliveries_with_suppliers.html', deliveries=deliveries, last_delivery_id=last_delivery_id)
    else:
          return "Вы не имеете прав доступа к этой странице."


@app.route('/user_orders_by_status', methods=['GET'])
@login_required
def user_orders_by_status():
    if current_user.role != 'admin':
      user = Client.query.filter_by(client_id=1).first()
      if user:
          ...
          with db.engine.connect() as connection:
                results = connection.execute(
                    db.text("SELECT * FROM get_user_orders_by_status(:client_id, :order_status);"),
                    {"client_id": user.client_id, "order_status": order_status},
                )
                orders_by_status = {
                'active': [],
                'completed': []
                ...
                if order_status == 'active':
                   for row in results:
                       if row.order_status_res in  ['В обработке','Новый']:
                         orders_by_status['active'].append(row)
                elif order_status == 'completed':
                   for row in results:
                       if row.order_status_res in  ['Доставлен', 'Выполнен']:
                           orders_by_status['completed'].append(row)
                else:
                     orders_by_status = results
                return render_template('user_orders_by_status.html', orders_by_status = orders_by_status, order_status = order_status)
      return "Пользователь не найден."
    else:
         return "Вы не имеете прав доступа к этой странице."


@app.route('/user_most_popular_products', methods=['GET'])
@login_required
def user_most_popular_products():
    if current_user.role != 'admin':
        user = Client.query.filter_by(client_id=1).first()
        if user:
           order_count = request.args.get('order_count')
           with db.engine.connect() as connection:
                results = connection.execute(
                    db.text("SELECT * FROM get_user_most_popular_products(:client_id, :order_count);"),
                    {"client_id": user.client_id, "order_count": order_count},
                )
                popular_products = {}
                for row in results:
                   popular_products[row.product_name] = row.order_count
           return render_template('user_most_popular_products.html', popular_products = popular_products, order_count=order_count)
        return "Пользователь не найден"
    else:
       return "Вы не имеете прав доступа к этой странице."
